...
class EquilibrationWaterkit:
    ...
    def run(self, pdb_file: str = None, run_id:str=None) -> None:
       ...
       # Add the required forces to the system
        ...
        #     logging.info("Running first minimization..")
        #     simulation.minimizeEnergy()
        #     logging.info(f"Current system's energy: {simulation.context.getState(getEnergy=True).getPotentialEnergy()}")
        # else:
        #     logging.info("Running enhanced first minimization..")
        # minim_scheme = self.minimization_scheme
        minim_scheme = [{ "name": "Stage 1", "forces": [100.0, 0.0] }]
        ...
        save_system(self.system, f"{self.out_dir}/system_solvated_{run_id}.xml")
        save_simulation(simulation, f"{self.out_dir}/checkpoint_solvated_{run_id}")
        save_pdb(self.topology, final_positions, f"{self.out_dir}/{run_id}_solvated.pdb")

        logging.info(f"Equilibration finished in {time.monotonic() - start_time:.2f} seconds.")
