...
def assess_symmetry(out_dir, x_min, x_max, x_grid_points, xCV_name, 
    ...
    for i, f in enumerate(files_fe):
        ...
        # # Plot and save heatmaps for the top half, bottom half, flipped bottom half, and their absolute difference
        # fig, axes = plt.subplots(1, 4, figsize=(24, 6))

        # sns.heatmap(top_half, cmap="Spectral", ax=axes[0])
        # axes[0].set_title(f"Top Half - {sys_name} - {walker_name}")
        # axes[0].set_xlabel(xCV_name)
        # axes[0].set_ylabel(yCV_name)

        # sns.heatmap(bottom_half, cmap="Spectral", ax=axes[1])
        # axes[1].set_title(f"Bottom Half - {sys_name} - {walker_name}")
        # axes[1].set_xlabel(xCV_name)
        # axes[1].set_ylabel(yCV_name)

        # sns.heatmap(bottom_half_flipped, cmap="Spectral", ax=axes[2])
        # axes[2].set_title(f"Flipped Bottom Half - {sys_name} - {walker_name}")
        # axes[2].set_xlabel(xCV_name)
        # axes[2].set_ylabel(yCV_name)

        # sns.heatmap(abs_diff, cmap="coolwarm", ax=axes[3])
        # axes[3].set_title(f"Abs Diff (Top vs Flipped Bottom) - {sys_name} - {walker_name}")
        # axes[3].set_xlabel(xCV_name)
        # axes[3].set_ylabel(yCV_name)

        # plt.tight_layout()
        # plt.savefig(f"{sys_name}_{walker_name}_heatmaps.png")
        # plt.close()
...
def compute_1d_pmf(F_2D, axis, kT=0.616):  # kT at 310K in kcal/mol
    """
    ...
    Parameters:
        ...
        kT (float): Thermal energy (k_B * T), default is 0.616 kcal/mol at 310K
...
def plot_FE_2D_with_marginals(out_dir, x_min, x_max, x_grid_points, xCV_name,
    ...
    for f in files_fe:
        ...
        F_x = compute_1d_pmf(F_2D, axis=0)  # 1D PMF along x
